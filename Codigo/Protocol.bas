        ...
        If Not .flags.Privilegios And PlayerType.User Then
            If MapaValido(map) And LenB(UserName) <> 0 Then
                ...
                ElseIf InMapBounds(map, X, Y) Then
                    If MapData(map, X, Y).UserIndex = 0 And MapData(map, X, Y).NpcIndex = 0 Then
                    ...
                    Else
                        Call WriteConsoleMsg(UserIndex, "no puedes transportarte en esa posicion", FontTypeNames.FONTTYPE_INFO)
                    End If
                End If
            End If
        End If
            ...
            Else
                If (.flags.Privilegios And (PlayerType.Dios Or PlayerType.Admin)) <> 0 Or _
                   (UserList(tUser).flags.Privilegios And (PlayerType.Consejero Or PlayerType.User)) <> 0 Then
                    If InMapBounds(.Pos.map, .Pos.X, .Pos.Y + 1) Then
                        If MapData(.Pos.map, .Pos.X, .Pos.Y + 1).UserIndex = 0 And MapData(.Pos.map, .Pos.X, .Pos.Y + 1).NpcIndex = 0 Then
                        ...
                        Else
                            Call WriteConsoleMsg(UserIndex, "Summon en posicion invalida, asegurese que no haya ningun personaje debajo de usted", FontTypeNames.FONTTYPE_FIGHT)
                        End If
                    End If
